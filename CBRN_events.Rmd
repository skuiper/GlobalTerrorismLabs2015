---
title: "Terrorism and Weapons of Mass Destruction (Solutions)"
...
#GTdata <- read.csv("FullGTD.csv", stringsAsFactors=FALSE)
GTdata <- read.csv("C:/Users/KUIPERS/Desktop/RStudio/CBRN/GTdata.csv")
#Remove the X column from the dataset
GTdata <- select(GTdata, -X)
...
CBRNIncidents <- GTdata[GTdata$WeaponType == "CBRN",]
...
GTDweap = summarise( group_by(GTdata, Year, WeaponType), Incidents=n())
...
GTDbyYear = summarise( group_by(GTdata, Year), TotalIncidents=n())
...
GTDdeath = summarise( group_by(GTdata, Year), TotalDeaths=sum(Fatalities, na.rm = TRUE))

#Sum Fatalities by weapon type and year
GTDweapDeath = summarise( group_by(GTdata, Year, WeaponType),  
